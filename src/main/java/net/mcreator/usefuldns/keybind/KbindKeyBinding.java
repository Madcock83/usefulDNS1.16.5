...
package net.mcreator.usefuldns.keybind;
...
import net.mcreator.usefuldns.procedures.KbindOnKeyPressedProcedure;
...
public class KbindKeyBinding extends UsefuldnsModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	...
	public void initElements() {
		keys = new KeyBinding("key.usefuldns.kbind", GLFW.GLFW_KEY_K, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					UsefuldnsMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				KbindOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
