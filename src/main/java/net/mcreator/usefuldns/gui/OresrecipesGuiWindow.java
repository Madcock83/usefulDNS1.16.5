...
@OnlyIn(Dist.CLIENT)
public class OresrecipesGuiWindow extends ContainerScreen<OresrecipesGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = OresrecipesGui.guistate;
	public OresrecipesGuiWindow(OresrecipesGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("usefuldns:textures/oresrecipes.png");
	@Override
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		...
		this.font.drawString(ms, "Anything with a smelt recipe will output x6", 2, 22, -12829636);
	}
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 224, this.guiTop + 6, 30, 20, new StringTextComponent("<"), e -> {
			if (true) {
				UsefuldnsMod.PACKET_HANDLER.sendToServer(new OresrecipesGui.ButtonPressedMessage(0, x, y, z));
				OresrecipesGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
	}
}
